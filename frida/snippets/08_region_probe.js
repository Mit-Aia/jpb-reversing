...
addrs.forEach((g, i) => {
  ...
  cnt[k] = 0;
  try { globalThis.__rl.push(Interceptor.attach(mod.base.add(g - 0x10000).add(1), { onEnter() { cnt[k]++; } })); }
  catch (e) { log("attach fail " + k + ": " + e.message); }
});
log("attached " + globalThis.__rl.length + " count hooks");
setTimeout(() => { log("FIRED(8s): " + JSON.stringify(Object.entries(cnt).filter(([k, v]) => v > 0))); }, 8000);
setTimeout(() => { log("FIRED(25s): " + JSON.stringify(Object.entries(cnt).filter(([k, v]) => v > 0))); globalThis.__rl.forEach(l => l.detach()); log("detached all"); }, 25000);
