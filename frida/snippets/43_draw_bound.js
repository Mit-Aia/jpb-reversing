...
const PRIMS = 0, N = 60, LO = 200, HI = 1500;   // PRIMS=0: log every draw with LO<=tris<=HI (battle: creatures unknown yet)
...
Interceptor.attach(mod.base.add(0x309fc4 - 0x10000).add(1), { onEnter(a) {
  if (shown >= N) return; const inner = a[1].add(4).readU32(); if (!inner || !cur[0]) return; const tris = ptr(inner).add(0x1c).readU32(); if (PRIMS ? tris !== PRIMS : (tris < LO || tris > HI)) return;
  const pos = m4[0] ? m4[0].slice(12, 15).join(",") : "?", key = tris + JSON.stringify(m4[3]) + JSON.stringify(m4[4]) + JSON.stringify(cur); if (seenPos[key]) return; seenPos[key] = 1; shown++;   // one log per distinct tile/texture (creatures may share the model matrix)
  log("BOUND_DRAW tris=" + tris + " boundTex=" + JSON.stringify(cur) + " worldPos=" + pos + " uv(loc3)=" + JSON.stringify(m4[3]) + " uv(loc4)=" + JSON.stringify(m4[4]));
} });
