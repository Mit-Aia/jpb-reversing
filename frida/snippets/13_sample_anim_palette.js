...
const stride = 4 + posBytes * 2 + bonesBytes * 2;
const out = new Uint8Array(SAMPLES * stride);
let k = 0;
const t0 = Date.now();
const timer = setInterval(() => {
  if (k >= SAMPLES) { clearInterval(timer); const f = new File(PKG + "anim2.bin", "wb"); f.write(out.buffer); f.flush(); f.close(); log("anim2.bin written samples=" + k + " stride=" + stride + " dur=" + (Date.now() - t0) + "ms"); return; }
  let o = k * stride;
